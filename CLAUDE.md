...
PromQueen is an LLM prompt engineering tool. It processes `.pqueen` prompt files to make LLM API calls. It has a VS Code extension in `vscode_extension/`, a one-shot runner in `promqueen.js` and an interactive CLI in `chat.js`.
