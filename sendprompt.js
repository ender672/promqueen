...
async function main() {
  ...
  if (options.attachment) {
    const resolvedAttachmentPath = path.resolve(options.attachment);
    const contents = fs.readFileSync(resolvedAttachmentPath, 'utf8');
    const attachmentPortion = `\nHere is the content of the attached file "${options.attachment}":\n\n<document>${contents}</document>`;
  ...
  for await (const event of getStream(response)) {
      ...
      if (fullConfig.pricing && json.usage) {
        process.stderr.write(usageToCostString(fullConfig.pricing, json.usage));
      }
