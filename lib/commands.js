...
function cmdSubmitText(ctx, text) {
    ...
    if (ctx.responseAs) {
        const overrideName = ctx.responseAs;
        nextEntry.name = overrideName;
        nextEntry.role = pqutils.PROMPT_ROLES.includes(overrideName) ? overrideName
            : overrideName === ctx.resolvedConfig.roleplay_user ? 'user'
            : 'assistant';
    }

    if (nextEntry.role === 'user') {
        ctx.setResponseAsOverride(null);
