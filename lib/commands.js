...
// All command handlers take a context object:
...
function cmdExit(ctx) {
    ...
    if (process.stderr.isTTY) process.stderr.write(`\nSaved to ${ctx.pqueenPath}\n`);
    ctx.exit();
}

function cmdShowPrompt(ctx) {
    const turn = prepareTurn(ctx.allMsgs, ctx.resolvedConfig, ctx.cwd);
...
function cmdRegenerate(ctx) {
    ...
    const turn = prepareTurn(priorMessages, ctx.resolvedConfig, ctx.cwd);
...
module.exports = { SLASH_COMMANDS, cmdSubmitText, findOpener };
