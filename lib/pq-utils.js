...
function guessNextSpeaker(history, userName) {
  ...
  if (lastSpeaker !== userName && lastSpeaker !== 'user') {
    ...
    const hasCharacterNames = history.some(m => !PROMPT_ROLES.includes(m.name));
...
function getDotConfigPath() {
  return path.join(os.homedir(), '.promqueen');
}

function ensureDotConfig() {
  const configPath = getDotConfigPath();
...
function ensureTemplateDir() {
  const templateDir = path.join(os.homedir(), '.promqueen-templates');
  if (fs.existsSync(templateDir)) return;
  fs.mkdirSync(templateDir);
  const src = path.join(__dirname, '..', 'templates', 'charcard-prompt-complete.jinja');
  const dest = path.join(templateDir, 'standard.jinja');
...
function updateDotConfig(updates) {
  const configPath = getDotConfigPath();
  if (!fs.existsSync(configPath)) {
    fs.copyFileSync(DOTCONFIG_TEMPLATE_PATH, configPath);
  }
...
// Generate a fully-documented ~/.promqueen config file.
