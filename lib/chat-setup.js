...
const { discoverTemplates, resolveTemplatePath, getTemplateDir } = require('./template-registry.js');
...
async function wizardSelectConnection(dotConfig, configDir) {
    const resolvedConfig = pqutils.resolveConfig({}, process.cwd(), {}, configDir);
    ...
    try {
        ...
        const existingProfiles = pqutils.loadDotConfig(configDir).connection_profiles || {};
        existingProfiles[selectedModel] = newProfile;
        pqutils.updateDotConfig({ connection_profiles: existingProfiles }, configDir);
...
async function wizardGetUserIdentity(dotConfig, configDir) {
    const currentUser = dotConfig.roleplay_user || '';
    const currentDesc = dotConfig.roleplay_user_description || '';

    // If a saved profile exists, offer it as a one-press selection
    if (currentUser && currentDesc) {
        const labels = [
            `${currentUser}`,
            '+ Create new user profile',
        ];
        const previews = [currentDesc, ''];
        const idx = await promptSelection(labels, 'User profile:', { previews });
        if (idx === 0) {
            return { userName: currentUser, userDescription: currentDesc };
        }
    }

    // Create new / no saved profile
    ...
    const dotConfigFile = pqutils.loadDotConfig(configDir);
    const updates = {};
    ...
    if (Object.keys(updates).length > 0) pqutils.updateDotConfig(updates, configDir);
...
async function wizardSelectTemplate(dotConfig, configDir) {
    ...
    const currentPath = currentTemplate ? resolveTemplatePath(currentTemplate, configDir) : null;
...
function createPqueenFile(pngPath, aiCardData, connectionName, userName, userDescription, roleplayGuidelines, charcardTemplate, configDir) {
    const dir = path.dirname(pngPath);
    ...
    const fallbackPath = path.join(getTemplateDir(configDir), 'standard.pqueen.jinja');
    ...
    const body = root.render(ctx).trimEnd();

    const yaml = require('js-yaml');
    const frontmatter = { roleplay_user: userName };
    if (connectionName) frontmatter.connection = connectionName;
    if (pngPath) frontmatter.charcard = path.basename(pngPath);
...
function testExistingConnection(pqueenPath, cliConfig, configDir) {
    ...
    let resolvedConfig;
    try {
        resolvedConfig = pqutils.resolveConfig(doc.config, cwd, cliConfig, configDir);
    } catch {
        return false;
    }
...
async function runSetup(pngPath, configDir) {
    ...
    if (!pqueenPath) {
        ...
        const { userName, userDescription } = await wizardGetUserIdentity(dotConfig, configDir);
        const templatePath = await wizardSelectTemplate(dotConfig, configDir);
